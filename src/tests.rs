...
fn data_layer_csv_deserialization() {
    let test_data =
        DataLayer::from_csv_file("data/testdata.csv").expect("Failed to load data/testdata.csv");
...
fn weight_layer_csv_and_get_methods() {
    let test_data = WeightLayer::from_csv_file("data/testweights.csv")
        .expect("Failed to load data/testweights.csv");
